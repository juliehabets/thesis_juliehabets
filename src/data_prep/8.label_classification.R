...
total_label <- total_label %>% group_by(artist, track_name) %>% count(label)
total_label <- total_label %>% group_by(artist, track_name) %>% mutate(max_n = max(n))

# keeping only most popular label
total_label <- total_label %>% group_by(artist, track_name) %>% filter(n == max_n)

# keeping only 1 label per artist in case when the max counts are the same
total_label <- total_label %>% group_by(artist, track_name) %>% slice(n=1)
